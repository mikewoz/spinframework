...
class vessLog : public ostream
{

public:
	// we initialize the ostream to use our logbuf
	vessLog(const char* logpath) : ostream(new logbuf(logpath))
	{
		buf = (logbuf*) rdbuf();
	}
...
// set the priority for a vessLog/logbuf this must be a global function and not
// a member to work around C++'s type resolution of overloaded functions
static vessLog& operator<<(vessLog& vlog, LogPriority pr)
{
