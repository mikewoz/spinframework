...
#ifndef __VideoNode_H
#define __VideoNode_H
 ...
 *        parent (which must be an ShapeNode for now)
 * 
 * This uses osg::ImageStream, so it can load several different movie formats,
 * including live feeds on the Mac via devices.live.
 */
class VideoNode : public ReferencedNode
{

public:

	VideoNode(SceneManager *sceneManager, char *initID);
	virtual ~VideoNode();
