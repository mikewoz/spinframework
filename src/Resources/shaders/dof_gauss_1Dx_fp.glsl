...
void main(void)
{
    ...
    for(float i=-lRadius; i<lRadius; i+=1.0)
    {
        ...
        //float lPixBlurRadius = radius * texture2D(texBlurMap, gl_TexCoord[0].st + vec2(i*lInputTexTexelWidth, 0)).r;

        //float lIsInBlur = 1.0; //step(lPixBlurRadius, lRadius);
        lTotalWeight += lWeight;
        lColor += texture2D(texColorMap, gl_TexCoord[0].st + vec2(i*lInputTexTexelWidth, 0)) * lWeight;
    }
    lColor /= lTotalWeight;
