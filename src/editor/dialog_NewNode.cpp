...
dialog_NewNode::dialog_NewNode(wxWindow* parent) : dialog_NewNode_base(parent)
{
    ...
    std::vector<std::string> nodeTypes = spinApp::Instance().sceneManager->getAllNodeTypes();
...
void dialog_NewNode::OnOK( wxCommandEvent& event )
{
    ...
    spinApp::Instance().SceneMessage("sss", "createNode", (char*)nodeID.char_str(), (char*)nodeType.char_str(), LO_ARGS_END);
    //spinApp::Instance().SceneMessage("sss", "createNode", nodeID.ToStdString(), nodeType.ToStdString(), LO_ARGS_END);
