...
*/
void wxSpinPropGrid::SetNode(spin::ReferencedNode* newNode, bool forceUpdate)
{
    ...
    if (newNode)
    {
        cppintrospection::Value pObjectValue = cppintrospection::Value(newNode);
        const Type& classType = pObjectValue.getInstanceType();

        if (!classType.isDefined())
        {
            std::cout << "ERROR: cppintrospection has no data for selected node." << std::endl;
            return;
        }
...
void wxSpinPropGrid::UpdateFromSpin()
{
    ...
    if (currentNode.valid())
    {
        ...
        while (nodeStateIterator != nodeState.end())
        {
            ...
            //wxPGProperty parentId = this->GetGrid()->GetRoot()->GetPropertyByName( wxString(parentString.c_str(), wxConvUTF8) );

            for (unsigned int i=0; i<GetGrid()->GetRoot()->GetChildCount(); i++)
            {
                wxPGProperty *parentId = GetGrid()->GetRoot()->Item(i)->GetPropertyByName( wxString((char*)args[0], wxConvUTF8) );
                if (parentId)
                {
 ...
 */
void wxSpinPropGrid::GenerateProperties(const cppintrospection::Type& classType, spin::ReferencedNode* pObject)
{
    ...
    wxPGProperty *categoryId = Append(new wxPropertyCategory(className));
    wxPGProperty *parentId = categoryId;
    ...
    for (MethodInfoList::const_iterator methodIter=methods.begin(); methodIter!=methods.end(); methodIter++)
    {
        ...
        if ( !method->isVirtual() && method->getReturnType().isVoid() && params.size() )
        {
            ...
            if (params.size() > 1)
            {
                ...
                parentId = AppendIn(categoryId, new wxPropertyCategory(methodName, wxPG_LABEL));
                ...
                if (param->getParameterType().isDefined())
                {
                    ...
                    else
                    {
                        ...
                        if (t=="bool") {
                            ...
                            // TODO: figure out attributes. Prior to wx 2.9:
                            /*
                            SetPropertyEditor(propId, wxPG_EDITOR(SpinCtrl));
                    ...
                    if (propId)
                    {
                        ...
                        {
                            helpText += wxT("OSC: /SPIN/") + spin::spinApp::Instance().getSceneID() + _("/") + wxString(pObject->id->s_name, wxConvUTF8);
            ...
            if (params.size() > 1)
            {
                wxString composedValue = parentId->GenerateComposedValue();
                parentId->SetValueFromString(composedValue);
                //std::cout << "composed value for " << methodName.mb_str() << "= " << composedValue.mb_str() << std::endl;

                helpText = wxT("");
                helpText += wxT("OSC: /SPIN/") + spin::spinApp::Instance().getSceneID() + wxT("/") + wxString(pObject->id->s_name, wxConvUTF8);
                helpText += wxT(" ") + methodName + wxT(" <");
                for (ParameterInfoList::const_iterator paramIter2=params.begin(); paramIter2!=params.end(); paramIter2++)
                {
                    helpText += wxT(" ") + wxString((*paramIter2)->getParameterType().getQualifiedName().c_str(), wxConvUTF8);
...
void wxSpinPropGrid::OnPropertyChanging(wxPropertyGridEvent& event)
{
    ...
    wxPGProperty *parent = event.GetMainParent();
	...
	spin::spinApp::Instance().NodeMessage(currentNode->id->s_name, msg);
 ...
 */
void wxProp_from_lo_message(wxPGProperty *parentId, const char *argTypes, int argc, lo_arg **argv)
{
    int i;
...
int wxSpinPropGrid_liblo_callback(const char *path, const char *types, lo_arg **argv, int argc, void *data, void *user_data)
{
    ...
    //wxPGProperty parentId = propGrid->GetGrid()->GetRoot()->GetPropertyByName( wxString(parentString.c_str(), wxConvUTF8) );

    for (unsigned int i=0; i < propGrid->GetGrid()->GetRoot()->GetChildCount(); i++)
    {
        wxPGProperty *parentId = propGrid->GetGrid()->GetRoot()->Item(i)->GetPropertyByName( wxString((char*)argv[0], wxConvUTF8) );
        if (parentId)
        {
            ...
            wxPGProperty *selectedProp = propGrid->GetGrid()->GetSelectedProperty();
            if (selectedProp)
            {
