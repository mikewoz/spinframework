...
BEGIN_VALUE_REFLECTOR(spin::SceneManager)
	...
	I_Method2(void, setShadowAmbientBias, IN, float, x, IN, float, y,
	          Properties::NON_VIRTUAL,
	          __void__setShadowAmbientBias__float__float,
	          "",
	          "The ambiantBias defines how much shadows darken the scene. It is usually used to set up lower bound for shadowing factor, in the range [0..1]. Sometimes we want to make shadows to be only a bit darker than lit areas. This is called \"AmbientBias\" because shadowed areas are lit only by the ambient light component). AmbientBias.y is usually set up to 1 - AmbientBias.x but it may be also set bit larger or smaller values used to make shadow range more dynamic or flat. ");
