...
class DebugVisitor : public osg::NodeVisitor
{
	public:
		...
		virtual void apply(osg::Group &node)
		{
			...
			} else if (dynamic_cast<ShapeNode*>(&node)) {
				osg::notify(osg::NOTICE) << leadingSpaces(getNodePath().size()) << "SHAPE: " << node.getName() << "  (" << node.getNumChildren () << " children)" << std::endl;
			} else if (dynamic_cast<ReferencedNode*>(&node)) {
				osg::notify(osg::NOTICE) << leadingSpaces(getNodePath().size()) << "ReferencedNode: " << node.getName() << "  (" << node.getNumChildren () << " children)" << std::endl;
			} else {
