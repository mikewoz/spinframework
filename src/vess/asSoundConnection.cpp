...
asSoundConnection::asSoundConnection (asSceneManager *s, osg::ref_ptr<asDSPnode> src, osg::ref_ptr<asDSPnode> snk)
{
	
	sceneManager = s;
	
	id = gensym( (string(src->id->s_name) + "-" + string(snk->id->s_name) + ".conn" ).c_str() );
	//id->s_thing = this; // can't do this because asSoundConnection doesn't extend asReferenced
	...
	sceneManager->sendSceneMessage("sss", "createNode", id->s_name, "asSoundConnection", LO_ARGS_END);	
...
asSoundConnection::~asSoundConnection()
{	
	//std::cout << "In asSoundConnection destructor... id: " << this->id->s_name << std::endl;
...
void asSoundConnection::updateSharedSpaces()
{
	vector<asSoundConnection*>::iterator iter1, iter2;
	asSoundConnection *conn;
			...
			if ((*iter1)->connectionType == SPACE_TO_NODE) // if the space is connected to a sink soundNode
			{
				for (iter2 = (*iter1)->sink->connectFROM.begin(); iter2 != (*iter1)->sink->connectFROM.end(); iter2++) // look at all the source nodes for that soundNode
				{
					if ((*iter2)->source == this->source) // if the source node is the same as the node in this connection
					{
						...
						conn->sharedSpaces.push_back( dynamic_cast<asSoundSpace*>(sink.get()) ); // add the shared space
						break;
					}
				}
			}
			...
			if ((*iter1)->connectionType == NODE_TO_SPACE) // if the connection comes from a source soundNode
			{
				for (iter2 = (*iter1)->source->connectTO.begin(); iter2 != (*iter1)->source->connectTO.end(); iter2++) // look at all the sink nodes for that source
				{
					if ((*iter2)->sink == this->sink) // if any of those sink nodes is the same as this connection's sink
					{
						...
						conn->sharedSpaces.push_back( dynamic_cast<asSoundSpace*>(source.get()) ); // add the shared space
						break;
					}
				}
			}
		...
		for (iter1 = source->connectTO.begin(); iter1 != source->connectTO.end(); iter1++) // look at all the nodes that the source is connected to
		{
			if ((*iter1)->connectionType == NODE_TO_SPACE) // if it's connected to a soundspace
			{
				for (iter2 = (*iter1)->sink->connectTO.begin(); iter2 != (*iter1)->sink->connectTO.end(); iter2++) // look at all the nodes that the soundspace is connected to
				{
					...
					{
						sharedSpaces.push_back( dynamic_cast<asSoundSpace*>( (*iter1)->sink.get() ) ); // add it
					}
				}
			}
		}
...
void asSoundConnection::computeWorldMatrices()
{
...
void asSoundConnection::computeDistance()
{	
	...
	if (connectionType==NODE_TO_SPACE) space = dynamic_cast<asSoundSpace*>( sink.get() );
	else if (connectionType==SPACE_TO_NODE) space = dynamic_cast<asSoundSpace*>( source.get() );
...
void asSoundConnection::computeRolloff()
{
	
	//std::cout << " asSoundConnection::computeRolloff (" << source->id->s_name << " - " << sink->id->s_name << ")" << std::endl;
	...
	osg::ref_ptr<asSoundNode> n;
	
	if (connectionType==NODE_TO_SPACE) {
		n = dynamic_cast<asSoundNode*>( source.get() );
		srcGain = n->rolloff->getValue( (srcIncidence * n->_spread) / osg::PI );
		snkGain = 1.0;
	} else if (connectionType==SPACE_TO_NODE) {
		n = dynamic_cast<asSoundNode*>( sink.get() );
...
void asSoundConnection::computeSeparation()
{
	//std::cout << " asSoundConnection::computeSeparation (" << source->id->s_name << " - " << sink->id->s_name << ")" << std::endl;
	...
	vector< osg::ref_ptr<asSoundSpace> >::iterator iter;
	asSoundConnection *space2node, *node2space;
...
void asSoundConnection::setThru (int newvalue)
{
...
void asSoundConnection::debug()
{
	...
	std::cout << "\nasSoundConnection [" << this->id << "]:" << std::endl;
 ...
 * 
 */
std::vector<lo_message> asSoundConnection::getState ()
{
...
// *****************************************************************************
void asSoundConnection::stateDump ()
{
	// NOTE: this operates differently than other VESS nodes. We dump the state
